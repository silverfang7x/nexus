...
export default function NexusGraph({
  ...
  useEffect(() => {
    ...
    const fileNodes = nodes.filter(n => n.type === 'file');
    const issueNodes = nodes.filter(n => n.type === 'issue');
    const svgHeight = mode === 'code' 
      ? Math.max(height, Math.max(fileNodes.length, issueNodes.length) * 52 + 160) 
      : height;
  ...
  return (
    ...
    >
      ...
      <svg 
        ...
        style={{
          height: mode === 'code' 
            ? Math.max(height, Math.max(nodes.filter(n => n.type === 'file').length, nodes.filter(n => n.type === 'issue').length) * 52 + 160) 
            : '100%'
        }}
        onClick={(event) => {
